...
const saveMyQuote = () => {
  const data = {
    ...
    mainboardId: post.value.quoteEntity.mainboard
      ? post.value.quoteEntity.mainboard.id
      : null,
    ...
    chassisId: post.value.quoteEntity.chassis
      ? post.value.quoteEntity.chassis.id
      : null,
    powerId: post.value.quoteEntity.power
      ? post.value.quoteEntity.power.id
      : null,
    coolerId: post.value.quoteEntity.cooler
      ? post.value.quoteEntity.cooler.id
      : null,
  };
  saveMyQuoteAPI(
    data,
  ...
  <v-container class="container">
    ...
    <v-row class="body">
      <div class="ml-5 my-5">
        <h2>
          <div
            v-for="(component, componentName) in post?.quoteEntity"
            :key="componentName"
          >
            <div>
    ...
    <v-row class="summery-info">
      <v-col cols="8">
        <h3>한 줄 설명 : {{ post?.summary }}</h3>
      </v-col>
      <v-col cols="2">
        <h3>작성자 : {{ post?.userNickname }}</h3>
      </v-col>
      <!-- <v-col
    ...
    <v-row>
      <v-col class="like">
        <v-col class="like-label" cols="5">
          <th>추천</th>
        </v-col>
        <v-col @click="selectlike(1)" cols="2">
      ...
      <v-col class="dislike">
        <v-col class="dislike-label" cols="5">
          <th>비추천</th>
        </v-col>
        <v-col @click="selectlike(-1)" cols="2">
    ...
    <Review class="input" :articleId="id" />
  </v-container>
</template>
